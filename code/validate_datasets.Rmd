...
# Exclusion restrtiction 

```{r}
# identity values = "Black or African American," "White," "Asian or Pacific Islander,"

study123 %>%
    filter(str_detect(identity, "Black")) %>%
    group_by(name) %>%
    summarize(pct_correct = mean(correct),
              pct_citizen = mean(citizen),
              pct_education = mean(education, na.rm = T),
              pct_income = mean(income, na.rm = T))
